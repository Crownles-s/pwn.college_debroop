...
## Solution:
-This requires us to run another cd command to change to the correct working directory.
- We must first get the working directory.
- Misunderstood a statement and tried to use ~ command to reveal working directory to reveal the working directory, did not work.
- As previously, ran /challenge/run to get the error to reveal the working directory.
...
### Notes:
- ~ command reveals the home directory.
...
## Solution:
- My first thought was that the answer is simply to run challenge/run. This did not work.
- So, I tried to go to root directory first, as the challenge description mentioned having to run challenge/run starting from root directory.
...
### Notes:
- Always try to go to the correct working directory before attempting to run any command unless specifically disallowed.
...
## Solution:
- My first thought was that the answer is simply to run .challenge/run. Again, this did not work.
- So, I tried to go to root directory as i did before, as the challenge description mentioned having to run challenge/run starting from root directory.
- That worked again.
...
### Notes:
- ALWAYS go to root directory first if mentioned.
...
## Solution:
- Enter the root directory, and try to run the challenge.
- The error told me where I went wrong, so I corrected myself and got the flag.
...
### Notes:
- Using the incorrect explicit naked paths can trigger similarly-named core commands.
- Failsafes stop this from happening.
...
## Solution:
- On first glance, I had absolutely no idea what to do.
- However, the "three characters or less" requirement gave me a clue, and i thought of ~/~.
- As onlt the leading ~ is expanded, the file would be saved in /home/hacker.
- Using ~/~ worked.
...
- Reddit post: https://www.reddit.com/r/linuxquestions/comments/kyo0kh/new_and_confused_what_does_the_tilde_sign/

### Notes:
- Only leading ~ are expanded.
